...
public extension Store {
    ...
    ///
    /// Reset-on-entry (`.onComingHome`) only clears an item on `arrivedHome`,
    /// and the exit handler that calls this never touches `lastEnteredHomeAt` —
    /// so `state(_:now:calendar:)` here can never be the departure this
